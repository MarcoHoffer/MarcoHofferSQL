...
--
-- Banco de dados: `marco_financeiro`
--
CREATE DATABASE IF NOT EXISTS `marco_financeiro` DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci;
USE `marco_financeiro`;
...
select 'O indentificador do registro não foi informado!' as 
Msg;
end if;
end$$
...
DROP PROCEDURE IF EXISTS `mySp_correntistaUpdade`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `mySp_correntistaUpdade` (`v_id` INT, `v_nome` VARCHAR(60), `v_cpf` VARCHAR(20))   BEGIN
...
(1, 'Marco Antonio Hoffer Madruga Filho', '023.456.789-10'),
